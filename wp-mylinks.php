 ...
 * @since             1.0.0
 * @package           Wp_Mylinks
 * @copyright  		  Copyright (c) 2020-2024, Walter Pinem, Seni Berpikir
 *
 * @wordpress-plugin
 ...
 */
define('WP_MYLINKS_VERSION', '1.0.7');
...
{
	wp_register_style('mylinks-public-css', plugin_dir_url(__FILE__) . 'public/css/wp-mylinks-public.min.css', array(), WP_MYLINKS_VERSION);
	wp_register_style('mylinks-youtube-css', plugin_dir_url(__FILE__) . 'public/css/wp-mylinks-youtube.min.css', array(), WP_MYLINKS_VERSION);
}
...
{
	wp_register_script('mylinks-public-js', plugin_dir_url(__FILE__) . 'public/js/wp-mylinks-public.js', array(), WP_MYLINKS_VERSION, true);
}
...
function wp_mylinks_settings_link($links_array, $plugin_file_name)
{
	if (strpos($plugin_file_name, basename(__FILE__))) {
		array_unshift($links_array, '<a href="edit.php?post_type=mylink&page=welcome">' . __('Settings', 'wp-mylinks') . '</a>');
	}
	return $links_array;
}
...
{
	return $original_value; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
